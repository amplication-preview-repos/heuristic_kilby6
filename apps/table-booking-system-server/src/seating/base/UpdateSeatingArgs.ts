/*
------------------------------------------------------------------------------ 
This code was generated by Amplication. 
 
Changes to this file will be lost if the code is regenerated. 

There are other ways to to customize your code, see this doc to learn more
https://docs.amplication.com/how-to/custom-code

------------------------------------------------------------------------------
  */
import { ArgsType, Field } from "@nestjs/graphql";
import { ApiProperty } from "@nestjs/swagger";
import { SeatingWhereUniqueInput } from "./SeatingWhereUniqueInput";
import { ValidateNested } from "class-validator";
import { Type } from "class-transformer";
import { SeatingUpdateInput } from "./SeatingUpdateInput";

@ArgsType()
class UpdateSeatingArgs {
  @ApiProperty({
    required: true,
    type: () => SeatingWhereUniqueInput,
  })
  @ValidateNested()
  @Type(() => SeatingWhereUniqueInput)
  @Field(() => SeatingWhereUniqueInput, { nullable: false })
  where!: SeatingWhereUniqueInput;

  @ApiProperty({
    required: true,
    type: () => SeatingUpdateInput,
  })
  @ValidateNested()
  @Type(() => SeatingUpdateInput)
  @Field(() => SeatingUpdateInput, { nullable: false })
  data!: SeatingUpdateInput;
}

export { UpdateSeatingArgs as UpdateSeatingArgs };
